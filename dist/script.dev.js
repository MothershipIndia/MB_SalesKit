...
$(function () {
  $(".residence_btn").click(function (event) {
    $(".residence").fadeIn(1000); // $('#media-video').play();
  });
}); //amenities Page
...
function IsFullScreenCurrently() {
  var full_screen_element = document.fullscreenElement || document.webkitFullscreenElement || document.mozFullScreenElement || document.msFullscreenElement || null; // If no element is in full-screen

  if (full_screen_element === null) return false;else return true;
}

$("#go-button").on('click', function () {
  if (IsFullScreenCurrently()) GoOutFullscreen();else GoInFullscreen($("body").get(0));
});
$("#fullscr-button").on('click', function () {
  GoInFullscreen($("body").get(0));
  $(".fullscreen").fadeOut(1);
});
